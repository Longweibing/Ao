...
//	 * @param bis 所有偏置值
//	 */
//	public static void forward(NeuralLayer[] layers, Map<Integer, double[][]> weights, List<List<Double>> bis) {	
//		for (int i = 0; i < layers.length; i++) {
//			if (i == 0) continue;
//			for (int j = 0; j < layers[i].size(); j++) {
//				double sum = 0;
//			    for (int z = 0; z < layers[i-1].size(); z++) {
//			    	sum += layers[i-1].getNeurals().get(z).getInputValue() * weights.get(i-1)[z][j];
//			    }
//			    if (i < layers.length-1)
//			    	sum += bis.get(i+1).get(j);
//			    sum = ActivationFunction.tanh(sum);
//			    layers[i].getNeurals().get(j).setInputValue(sum);
//			}
//		}
//	}
	 ...
	 * @param alphe 学习率
	 */
//	public static void backward(NeuralLayer[] layers, List<Double> target, Map<Integer, double[][]> weights, List<List<Double>> bis, String flag, double alphe) {
//		
//		// 多标签分类的反向传播算法
...
//			for (int i = 0; i < target.size(); i++) {
//				double x = layers[layers.length-1].getNeurals().get(i).getInputValue();
//				double y = 0;
//				// 该标签为正例
//				if (target.get(i) == 1) {			
//					for (int j = 0; j < couExample.size(); j++) {
//						double z = layers[layers.length-1].getNeurals().get(couExample.get(j)).getInputValue();
//						y += Math.exp(-(x - z));
//					}
//					y *= (1 / (double)posExample.size() * couExample.size()) * ((1 + x) * (1 - x));
//				} else {  // 该标签为反例
//					for (int j = 0; j < posExample.size(); j++) {
//						double z = layers[layers.length-1].getNeurals().get(posExample.get(j)).getInputValue();
//						y += Math.exp(-(x - z));
//					}
...
//				for (int j = 0; j < layers[i].size(); j++) {
//					double e = 0;
//					double x = layers[i].getNeurals().get(j).getInputValue();
...
//						x[j][k] -= alphe * map.get(n).get(k) * layers[i].getNeurals().get(j).getInputValue();						
//					}
//				}
//				n++;
//			}
...
//		NeuralLayer[] layers = new NeuralLayer[] {
