...
public class DNNMLL extends MultiLabelLearnerBase {

	private static final long serialVersionUID = -4202757063777778115L;
    ...
    public DNNMLL(long randomnessSeed) {
	...
	private NeuralNet buildNeuralNetwork(int inputsDim) {
        ...
        if (hiddenLayersTopology == null) {
        	// hidden layer,its the number of layer changes from curnum to Math.round(0.2f * inputsDim), Intervals of 1
        	int curnum = inputsDim - 1;
        	int minhdim = Math.round(0.8f * inputsDim);
        	hiddenLayersTopology = new int[curnum - minhdim + 1];
        	int index = 0;
        	// Layer build
            while (curnum >= minhdim) {
            	hiddenLayersTopology[index++] = curnum--;
            }
            networkTopology = new int[hiddenLayersTopology.length + 2];
            for (int i = 0, j = 0; i < networkTopology.length; i++) {
            	if (i == 0) {
            		networkTopology[i] = inputsDim;
            	} else if (i == networkTopology.length - 1) {
            		networkTopology[i] = numLabels;
            	} else {
            		networkTopology[i] = hiddenLayersTopology[j++];
            	}
            }
        } else {
            networkTopology = new int[hiddenLayersTopology.length + 2];
